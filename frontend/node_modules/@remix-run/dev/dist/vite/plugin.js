...
const supportedRemixConfigKeys = ["appDirectory", "assetsBuildDirectory", "future", "ignoredRouteFiles", "publicPath", "routes", "serverModuleFormat"];
...
const remixVitePlugin = (options = {}) => {
  ...
  let resolveServerBuildConfig = () => {
    if (!("__remixServerBuildConfig" in viteUserConfig) || !viteUserConfig.__remixServerBuildConfig) {
      return null;
    }
    let {
      routes,
  ...
  let resolvePluginConfig = async () => {
    ...
    let resolvedRemixConfig = await config.resolveConfig(pick__default["default"](pluginConfig, supportedRemixConfigKeys), {
      rootDirectory,
      isSpaMode
    });
    ...
    if (isSpaMode && unstable_serverBundles) {
      console.warn(pc__default["default"].yellow(pc__default["default"].bold("⚠️  SPA Mode: ") + "the `unstable_serverBundles` config is invalid with " + "`unstable_ssr:false` and will be ignored`"));
  ...
  return [{
    ...
    writeBundle: {
      ...
      async handler() {
        ...
        if (cachedPluginConfig.isSpaMode) {
          await handleSpaMode(path__namespace.join(rootDirectory, serverBuildDirectory), serverBuildFile, assetsBuildDirectory, viteConfig);
        }
      }
    },
    async buildEnd() {
    ...
    async transform(code, id, options) {
      ...
      if (pluginConfig.isSpaMode) {
        ...
        if (serverOnlyExports.length > 0) {
          ...
          let message = `SPA Mode: ${serverOnlyExports.length} invalid route export(s) in ` + `\`${route.file}\`: ${str}. See https://remix.run/future/spa-mode ` + `for more information.`;
          throw Error(message);
        }
        if (route.id !== "root") {
          let hasHydrateFallback = esModuleLexer.parse(code)[1].map(exp => exp.n).some(exp => exp === "HydrateFallback");
          if (hasHydrateFallback) {
            let message = `SPA Mode: Invalid \`HydrateFallback\` export found in ` + `\`${route.file}\`. \`HydrateFallback\` is only permitted on ` + `the root route in SPA Mode. See https://remix.run/future/spa-mode ` + `for more information.`;
            throw Error(message);
          }
        }
      }
    ...
    async handleHotUpdate({
      ...
      if (route) {
        ...
        let newRouteMetadata = await getRouteMetadata(pluginConfig, viteChildCompiler, route);
...
async function handleSpaMode(serverBuildDirectoryPath, serverBuildFile, assetsBuildDirectory, viteConfig) {
  ...
  await fse__namespace.writeFile(path__namespace.join(assetsBuildDirectory, "index.html"), html);
  viteConfig.logger.info("SPA Mode: index.html has been written to your " + pc__default["default"].bold(path__namespace.relative(process.cwd(), assetsBuildDirectory)) + " directory");
