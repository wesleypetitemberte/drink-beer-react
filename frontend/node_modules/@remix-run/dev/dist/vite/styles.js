...
const getStylesForFiles = async (viteDevServer, config, cssModulesManifest, files) => {
  let styles = {};
  let deps = new Set();
  try {
    for (let file of files) {
      ...
      if (!node) {
        try {
          await viteDevServer.transformRequest(resolveFileUrl.resolveFileUrl(config, normalizedPath));
        } catch (err) {
          console.error(err);
        }
...
const getStylesForUrl = async (viteDevServer, config, cssModulesManifest, build, url) => {
  var _matchRoutes;
  ...
  let styles = await getStylesForFiles(viteDevServer, config, cssModulesManifest, [
  // Always include the client entry file when crawling the module graph for CSS
  path__namespace.relative(config.rootDirectory, config.entryClientFilePath),
